#!/usr/bin/env python3
"""
POKER COACH PRO - SISTEMA DEFINITIVO CORREGIDO
...
class PokerCoachDefinitiveFixed:
    """Sistema definitivo CORREGIDO"""
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # Mostrar estadísticas cada 3 manos
                if self.hand_count % 3 == 0:
                    self.show_stats()
                
                # Pausa entre manos
                print(f"\n Próxima mano en 5s... (Ctrl+C para menú)")
                ...
                print(f"\n  Error: {e}")
                print("Continuando con siguiente mano...")
                time.sleep(2)
    
    def show_header(self):
        """Mostrar encabezado"""
        print("=" * 70)
        print(" POKER COACH PRO - SISTEMA DEFINITIVO CORREGIDO")
        print("=" * 70)
    ...
    def generate_situation(self):
        """Generar situación realista CORREGIDA"""
        # Usar letras para palos en lugar de símbolos unicode
        ...
        # Cartas del héroe - asegurar que son strings de 2 caracteres
        card1 = deck.pop()
        card2 = deck.pop()
    ...
    def show_situation(self, situation):
        """Mostrar situación CORREGIDA"""
        print(f"\n SITUACIÓN ACTUAL:")
        print(f"   Posición: {situation['position']} ({situation['position_desc']})")
        print(f"   Calle: {situation['street']} - {situation['street_desc']}")
        
        # Mostrar cartas con símbolos bonitos
        card1 = situation['hero_cards'][0]
        card2 = situation['hero_cards'][1]
        
        # Convertir letras de palos a símbolos para display
        suit_symbols = {'s': '', 'h': '', 'd': '', 'c': ''}
        suit_colors = {'s': '', 'h': '', 'd': '', 'c': ''}
        
        card1_display = f"{card1[0]}{suit_symbols.get(card1[1], card1[1])}"
        card2_display = f"{card2[0]}{suit_symbols.get(card2[1], card2[1])}"
        
        print(f"   Tus cartas: {card1_display}  {card2_display}")
        
        if situation['community_cards']:
            community_display = []
            for card in situation['community_cards']:
                community_display.append(f"{card[0]}{suit_symbols.get(card[1], card[1])}")
            print(f"   Mesa: {'  '.join(community_display)}")
        else:
    ...
    def analyze_situation(self, situation):
        """Analizar situación y tomar decisión GTO - CORREGIDO"""
        cards = situation['hero_cards']
        
        # Asegurar que las cartas tienen el formato correcto
        if len(cards) < 2:
            return self.default_decision(situation)
        
        # Verificar formato de cartas
        for card in cards:
            if len(card) != 2:
                return self.default_decision(situation)
        ...
        # Calcular pot odds para ajuste de call
        if situation['pot'] > 0 and to_call > 0:
            pot_odds = to_call / (situation['pot'] + to_call)
            call_score = 1.0 - pot_odds  # Mientras mejores odds, mayor score
        else:
            call_score = 1.0
    ...
    def default_decision(self, situation):
        """Decisión por defecto si hay algún error"""
        actions = situation['actions_available']
        action = random.choice(actions) if actions else 'FOLD'
        
        return {
            'action': action,
            'confidence': random.randint(50, 70),
            'reason': 'Decisión balanceada basada en situación',
            'alternatives': [],
            'score': 0.5
        }
    
    def evaluate_hand(self, cards):
        """Evaluar fuerza de mano (0-1) - CORREGIDO"""
        if len(cards) < 2:
            return 0.3
        
        card1, card2 = cards[0], cards[1]
        
        # Verificar que las cartas tengan formato correcto
        ...
        # Verificar que los ranks sean válidos
        if rank1 not in values or rank2 not in values:
            return 0.3
    ...
    def show_stats(self):
        ...
        if self.hand_count == 0:
            print("  Aún no se han jugado manos")
            return
        
        print(f"  Manos jugadas: {self.hand_count}")
        print(f"  Folds: {self.stats['folds']} ({self.stats['folds']/self.hand_count*100:.1f}%)")
        print(f"  Calls: {self.stats['calls']} ({self.stats['calls']/self.hand_count*100:.1f}%)")
        print(f"  Raises/Bets: {self.stats['raises']} ({self.stats['raises']/self.hand_count*100:.1f}%)")
        print(f"  Checks: {self.stats['checks']} ({self.stats['checks']/self.hand_count*100:.1f}%)")
        
        if self.stats['allins'] > 0:
            print(f"  All-Ins: {self.stats['allins']} ({self.stats['allins']/self.hand_count*100:.1f}%)")
    ...
    def show_final_stats(self):
        ...
        if self.hand_count > 0:
            ...
            # Consejo final
            print(f"\n CONSEJO FINAL:")
            if self.stats['folds'] / self.hand_count > 0.6:
                print("  Considera jugar más manos - puedes estar foldando demasiado")
            elif self.stats['raises'] / self.hand_count < 0.2:
...
def main():
    """Función principal"""
    try:
        coach = PokerCoachDefinitiveFixed()
        coach.run()
    except KeyboardInterrupt:
